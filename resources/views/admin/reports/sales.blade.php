...
<form class="row g-2 mb-3" method="get">
    ...
    <div class="col-md-4"><select class="form-select" name="event_id"><option value="">Semua event</option>@foreach($events as $event)<option value="{{ $event->id }}" @selected(request('event_id')==$event->id)>{{ $event->title }}</option>@endforeach</select></div>
...
<div class="card border-0 shadow-sm"><div class="table-responsive">
    <table class="table mb-0"><thead><tr><th>Kode</th><th>Event</th><th>Total</th><th>Paid At</th></tr></thead><tbody>
    @foreach($transactions as $transaction)<tr><td>{{ $transaction->code }}</td><td>{{ $transaction->event()?->title }}</td><td>Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</td><td>{{ $transaction->paid_at?->format('d M Y H:i') }}</td></tr>@endforeach
    </tbody></table>
</div></div>
